...
/**
 * Annotation to create [org.camunda.community.extension.coworker.zeebe.worker.JobCoworker]. Works closely with Spring Zeebe.
 *
 ...
 * that we take by getting [org.springframework.beans.factory.config.ConfigurableBeanFactory.getBeanExpressionResolver] from the context.
 *
...
annotation class Coworker(
    /**
     * Coworker's type.
     * By default, we are getting it from the configuration properties bean's method [io.camunda.zeebe.spring.client.properties.ZeebeClientConfigurationProperties.getDefaultJobWorkerType]
     * and if it is null we are getting it by accessing the [io.camunda.zeebe.spring.client.bean.MethodInfo.getMethodName] method.
     *
     * There is an object exists in the evaluation context with type [io.camunda.zeebe.spring.client.bean.MethodInfo] with the key `methodInfo`.
    ...
    /**
     * Coworker's type.
     * By default, we are getting it from the configuration properties bean's method [io.camunda.zeebe.spring.client.properties.ZeebeClientConfigurationProperties.getDefaultJobWorkerName]
     ...
     * The evaluated value should be [String].
     */
    val name: String = "#{$ZEEBE_CLIENT_CONFIGURATION_PROPERTIES.getDefaultJobWorkerName() ?: methodInfo.getBeanName() + '#' + methodInfo.getMethodName()}",
    /**
     * Coworker's job timeout.
     * By default, we are getting it from the configuration properties bean's method [io.camunda.zeebe.spring.client.properties.ZeebeClientConfigurationProperties.getDefaultJobTimeout].
     *
     * In the evaluation context, two objects exist:
     * 1. Key: `methodInfo`, value type: [io.camunda.zeebe.spring.client.bean.MethodInfo],
     * 2. Key: `type`, value type: [String] - this is the current [Coworker.type].
     ...
     * By default, we are getting it from the configuration properties bean's method [io.camunda.zeebe.spring.client.properties.ZeebeClientConfigurationProperties.getDefaultJobWorkerMaxJobsActive].
     *
     * In the evaluation context, two objects exist:
     * 1. Key: `methodInfo`, value type: [io.camunda.zeebe.spring.client.bean.MethodInfo],
     * 2. Key: `type`, value type: [String] - this is the current [Coworker.type].
     ...
     * By default, we are getting it from the configuration properties bean's method [io.camunda.zeebe.spring.client.properties.ZeebeClientConfigurationProperties.getRequestTimeout].
     *
     * In the evaluation context, two objects exist:
     * 1. Key: `methodInfo`, value type: [io.camunda.zeebe.spring.client.bean.MethodInfo],
     * 2. Key: `type`, value type: [String] - this is the current [Coworker.type].
     ...
     * By default, we are getting it from the configuration properties bean's method [io.camunda.zeebe.spring.client.properties.ZeebeClientConfigurationProperties.getDefaultJobPollInterval].
     *
     * In the evaluation context, two objects exist:
     * 1. Key: `methodInfo`, value type: [io.camunda.zeebe.spring.client.bean.MethodInfo],
     * 2. Key: `type`, value type: [String] - this is the current [Coworker.type].
