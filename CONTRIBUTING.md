...
If you have additional information that can help us understand the bug's behavior, how it presents in your configuration, or its impact, please add that as a comment.

### New bug reports

If no issue already exists for the bug, please create a new one.

When creating a bug report, please fill out the template. What helps us are the steps for us to be able to reproduce the problem in front of us.

To generate these, start from nothing, and document the steps required to set up a project that shows the bug. If you create such a project as a new GitHub repo, you will have a Minimal Reproducible Example. We can then check out that project and see the bug in front of us immediately. This will increase the speed that we can address the issue. It will also help you isolate the actual problem, and sometimes fix it.
...
If you have additional information that can help us understand the behavior you need, or another use case not covered in the existing comments, please add that as a comment.

### New feature requests

If no issue already exists for the feature request, please create a new one.

When creating a feature request, please fill out the template. What helps us are both the motivation for the feature (what is your use case and what you want to achieve), as well as what you would like the feature to be. Sometimes there is an existing way to accomplish what you want, and we may be able to recommend that.
...
Maybe while getting started, you notice a step or a dependency that we missed out, on or something that would have helped you. In that case, opening a Pull Request with a patch that adds it will help others when they get started. They may never know to thank you for it, but we will!
...
If you are implementing a feature, it is a good idea to comment on the issue with your proposed approach. Early feedback and coordination increase the chances that we can merge it.
...
Maybe you implement a missing feature that you want. That's the beauty of open source - we co-create it. To get that merged into the code base, open a feature request issue. It's a good idea to discuss your proposed approach with us in a feature request issue. We might be planning to do it already and have an idea, or maybe we can help you identify the best approach given our familiarity with the codebase.
...
Right now there is no strict code style checking in this project. If you are using IntelliJ IDEA the code style is shared within the project.

## Commit Message Guidelines

Use the `feat` for feature requests, `fix` for bug fixes, and `chore` for internal stuff.
Also, make sure to put the GitHub issue identifier in the commit messages.
I.e.:
```
feat(#123): Implement the new API method

I need to implement the new API method because I need to use the Reactor project instead of Kotlin Coroutines.
```
...
If you want to build and use this library locally in another Maven/Gradle project, just run `mvn clean install`.
